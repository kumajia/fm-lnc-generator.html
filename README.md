...
Football Manager のクラブ名・大会名を日本語カタカナに自動翻訳し、`.lnc` コードを生成するツールです。
...
各エントリについて、次の2種類のコードを出力します。出力するコードの種類は「対象」（クラブ / 大会）で切り替わります。
...
- **クラブ / 大会の切り替え**: 「対象」で切り替えると、出力が `CLUB_…` / `COMP_…` に変わります。どちらも翻訳・SHORT名編集・スクショ抽出に対応。
